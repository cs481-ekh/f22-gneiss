...
test("400 response creates alert", async () => {
  ...
  let file = new File(["blob"], "coolDocument.pdf", { type: "application/pdf" });
  userEvent.upload(screen.getByTestId("fileSelect"), file)
  fireEvent.click(screen.getByText("Save"));
  await waitFor(() => expect(screen.getByText("Failed to save PDF")).toBeTruthy());
});
